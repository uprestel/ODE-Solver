...
This example solves a BVP
                ...
                (   u_2(t)  )
        u'(t) = ( 3/4 t(t-1))      for t ∈ [0,3]

        u_1(0) = 0,   u_1(1) = 1.6875

        which has the exact solution u_1(t) = (t/4)^4 - (t/2)^3
"""
...
def f(t, y):
    return np.array([y[1], 3./4*(t-1)*t])


def r(s0, sm):
    return np.array([s0[0], sm[0] - 1.6875])
...
if __name__ == "__main__":
    m = 20
    maxiter = 5
    t = np.linspace(0., 3., m + 1)

    bv_left = np.array([0, 0])
    bv_right = np.array([1.6875, 0])
    ...
    integrator.run(f, 0, np.array([0.0, 1.36188272e-16]), 3)
