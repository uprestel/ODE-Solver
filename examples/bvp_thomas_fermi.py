...
if __name__ == "__main__":
    ...
    bv_left = np.array([1, 0])    # left boundary value
    bv_right = np.array([0, 0])             # right boundary value
    interpolated_bv = util.getInterpolatedVectors(bv_right, bv_left, m + 1)

    Ba = np.matrix([[1, 0], [0, 0]])        # Jacobian of r(.,.) w.r.t s0
    Bb = np.matrix([[0, 0], [1, 0]])        # Jacobian of r(.,.) w.r.t sm
    integrator = erk.DormandPrince(h0=.1, hmin=0.1, hmax=0.3, epsilon=1e-6)

    # standard stuff: classic explicit DormandPrince and standard Newton.
    # integrator = rk.DormandPrince(h=.1)
