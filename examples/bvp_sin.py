...
This example solves a BVP
                ...
                (   u_2(t)  )
        u'(t) = ( -λ^3 * sin(λt))      for t ∈ [0,12]

        u_1(0) = 0,   u_1(1) = λ*sin(12*λ)

        which has the exact solution u_1(t) = λ*sin(λt)
"""
...
def f(t, y):
    return np.array([y[1], -lambd ** 3 * sin(lambd * t)])


def r(s0, sm):
    return np.array([s0[0], sm[0] - lambd * sin(lambd * 12)])


def exact_f(t):
    return lambd * sin(lambd * t)
...
if __name__ == "__main__":
    ...
    bv_right = np.array([lambd * sin(lambd * 12), 0])
    ...
    integrator = erk.DormandPrince(h0=.1, hmin=0.01, hmax=0.2, epsilon=1e-6)
    ...
    integrator.run(f, 0, np.array([ 0., 2.25]), 12)
